...
public class XSeriesBirender {
	public static void main(String[] args) {
		Map<Integer, List<Integer>> map = new HashMap<>();

		String str = "11,5,19,2,8,3,4,30"; // another sample input 67,32,1
		...
		for (int i = 1; i < list.size(); i++) {
			...
			if (!outList.isEmpty()) {
				map.put(outList.size(), outList); // new list of same size (key) will replace old, we do so to get first
													// list element
			}
